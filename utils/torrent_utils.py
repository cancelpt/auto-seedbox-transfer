...
class TorrentFile:
    ...
    def change_announce(self, announce_list):
        ...
        if cleaned_list:
            self.torrent_data[b"announce-list"] = [[x.encode() for x in cleaned_list]]
    ...
    def add_trackers(self, new_trackers: list[str]):
        ...
        current_trackers = [t.strip() for t in self.trackers if t and t.strip()] if self.trackers else []
...
def export_as_torrent(
    torrent_data,
    bt_announce_list,
    source="pt2bt",
    comment="BT is Best Taste!",
    is_private=False,
    created_by="pt2bt v0.1",
    creation_date=int(time.time()),
    output_name="",
    path: str = "",
) -> tuple[bool, str, "TorrentFile"]:
